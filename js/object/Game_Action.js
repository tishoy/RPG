...
export default class Game_Action {
    ...
    subject() {
        if (this._subjectActorId > 0) {
            return GameGlobal.$gameActors.actor(this._subjectActorId);
        } else {
            return GameGlobal.$gameTroop.members()[this._subjectEnemyIndex];
        }
    }
    ...
    setSkill(skillId) {
        this._item.setObject(GameGlobal.$dataSkills[skillId]);
    }

    setItem(itemId) {
        this._item.setObject(GameGlobal.$dataItems[itemId]);
    }
    ...
    isAttack() {
        return this.item() === GameGlobal.$dataSkills[this.subject().attackSkillId()];
    }

    isGuard() {
        return this.item() === GameGlobal.$dataSkills[this.subject().guardSkillId()];
    }

    isMagicSkill() {
        if (this.isSkill()) {
            return GameGlobal.$dataSystem.magicSkills.contains(this.item().stypeId);
        } else {
            return false;
        }
    }
    ...
    testApply(target) {
        return (this.isForDeadFriend() === target.isDead() &&
            (GameGlobal.$gameParty.inBattle() || this.isForOpponent() ||
    ...
    applyGlobal() {
        this.item().effects.forEach(function (effect) {
            if (effect.code === Game_Action.EFFECT_COMMON_EVENT) {
                GameGlobal.$gameTemp.reserveCommonEvent(effect.dataId);
            }
        }, this);
    }
}