...
export default class Game_BattlerBase {
    ...
    resetStateCounts(stateId) {
        var state = GameGlobal.$dataStates[stateId];
        var variance = 1 + Math.max(state.maxTurns - state.minTurns, 0);
        this._stateTurns[stateId] = state.minTurns + Math.randomInt(variance);
        this._stateSteps[stateId] = GameGlobal.$dataStates[stateId].stepsToRemove || 0;
    }
    ...
    buffIconIndex(buffLevel, paramId) {
        if (buffLevel > 0) {
            return this.ICON_BUFF_START + (buffLevel - 1) * 8 + paramId;
        } else if (buffLevel < 0) {
            return this.ICON_DEBUFF_START + (-buffLevel - 1) * 8 + paramId;
        } else {
            return 0;
        }
    }
    ...
    paramRate(paramId) {
        return this.traitsPi(this.TRAIT_PARAM, paramId);
    }
    ...
    xparam(xparamId) {
        return this.traitsSum(this.TRAIT_XPARAM, xparamId);
    }

    sparam(sparamId) {
        return this.traitsPi(this.TRAIT_SPARAM, sparamId);
    }

    elementRate(elementId) {
        return this.traitsPi(this.TRAIT_ELEMENT_RATE, elementId);
    }

    debuffRate(paramId) {
        return this.traitsPi(this.TRAIT_DEBUFF_RATE, paramId);
    }

    stateRate(stateId) {
        return this.traitsPi(this.TRAIT_STATE_RATE, stateId);
    }

    stateResistSet() {
        return this.traitsSet(this.TRAIT_STATE_RESIST);
    }
    ...
    attackElements() {
        return this.traitsSet(this.TRAIT_ATTACK_ELEMENT);
    }

    attackStates() {
        return this.traitsSet(this.TRAIT_ATTACK_STATE);
    }

    attackStatesRate(stateId) {
        return this.traitsSum(this.TRAIT_ATTACK_STATE, stateId);
    }

    attackSpeed() {
        return this.traitsSumAll(this.TRAIT_ATTACK_SPEED);
    }

    attackTimesAdd() {
        return Math.max(this.traitsSumAll(this.TRAIT_ATTACK_TIMES), 0);
    }

    addedSkillTypes() {
        return this.traitsSet(this.TRAIT_STYPE_ADD);
    }

    isSkillTypeSealed(stypeId) {
        return this.traitsSet(this.TRAIT_STYPE_SEAL).contains(stypeId);
    }

    addedSkills() {
        return this.traitsSet(this.TRAIT_SKILL_ADD);
    }

    isSkillSealed(skillId) {
        return this.traitsSet(this.TRAIT_SKILL_SEAL).contains(skillId);
    }

    isEquipWtypeOk(wtypeId) {
        return this.traitsSet(this.TRAIT_EQUIP_WTYPE).contains(wtypeId);
    }

    isEquipAtypeOk(atypeId) {
        return this.traitsSet(this.TRAIT_EQUIP_ATYPE).contains(atypeId);
    }

    isEquipTypeLocked(etypeId) {
        return this.traitsSet(this.TRAIT_EQUIP_LOCK).contains(etypeId);
    }

    isEquipTypeSealed(etypeId) {
        return this.traitsSet(this.TRAIT_EQUIP_SEAL).contains(etypeId);
    }

    slotType() {
        var set = this.traitsSet(this.TRAIT_SLOT_TYPE);
    ...
    actionPlusSet() {
        return this.traits(this.TRAIT_ACTION_PLUS).map(function (trait) {
            return trait.value;
        });
    }

    specialFlag(flagId) {
        return this.traits(this.TRAIT_SPECIAL_FLAG).some(function (trait) {
            return trait.dataId === flagId;
        });
    }

    collapseType() {
        var set = this.traitsSet(this.TRAIT_COLLAPSE_TYPE);
        return set.length > 0 ? Math.max.apply(null, set) : 0;
    }

    partyAbility(abilityId) {
        return this.traits(this.TRAIT_PARTY_ABILITY).some(function (trait) {
            return trait.dataId === abilityId;
        });
    }

    isAutoBattle() {
        return this.specialFlag(this.FLAG_ID_AUTO_BATTLE);
    }

    isGuard() {
        return this.specialFlag(this.FLAG_ID_GUARD) && this.canMove();
    }

    isSubstitute() {
        return this.specialFlag(this.FLAG_ID_SUBSTITUTE) && this.canMove();
    }

    isPreserveTp() {
        return this.specialFlag(this.FLAG_ID_PRESERVE_TP);
    }
    ...
    sortStates() {
        this._states.sort(function (a, b) {
            var p1 = GameGlobal.$dataStates[a].priority;
            var p2 = GameGlobal.$dataStates[b].priority;
            if (p1 !== p2) {
                return p2 - p1;
            }
            return a - b;
    ...
    meetsItemConditions(item) {
        return this.meetsUsableItemConditions(item) && GameGlobal.$gameParty.hasItem(item);
    }
    ...
    canAttack() {
        return this.canUse(GameGlobal.$dataSkills[this.attackSkillId()]);
    }

    canGuard() {
        return this.canUse(GameGlobal.$dataSkills[this.guardSkillId()]);
    }
}